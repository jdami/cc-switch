...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    5 => {
                        log::info!("迁移数据库从 v5 到 v6（Antigravity 支持）");
    ...
    /// v5 -> v6 迁移：添加 Antigravity 支持
    ///
    /// 为 mcp_servers 和 skills 表添加 enabled_antigravity 列。
    fn migrate_v5_to_v6(conn: &Connection) -> Result<(), AppError> {
        // 为 mcp_servers 表添加 enabled_antigravity 列
        Self::add_column_if_missing(
            conn,
            "mcp_servers",
            "enabled_antigravity",
        ...
        // 为 skills 表添加 enabled_antigravity 列
        Self::add_column_if_missing(
            conn,
            "skills",
            "enabled_antigravity",
        ...
        log::info!("v5 -> v6 迁移完成：已添加 Antigravity 支持");
        Ok(())
    }
