...
impl McpApps {
    ...
    pub fn is_empty(&self) -> bool {
        !self.claude && !self.codex && !self.gemini && !self.opencode && !self.antigravity
    }
}
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match normalized.as_str() {
            ...
            other => Err(AppError::localized(
                "unsupported_app",
                format!("不支持的应用标识: '{other}'。可选值: claude, codex, gemini, opencode, antigravity。"),
                format!("Unsupported app id: '{other}'. Allowed: claude, codex, gemini, opencode, antigravity."),
            )),
        }
    }
}
...
impl MultiAppConfig {
    ...
    /// 将 v3.6.x 的分应用 MCP structure 迁移到 v3.7.0 的统一结构
    ///
    /// 迁移策略：
    ...
    pub fn migrate_mcp_to_unified(&mut self) -> Result<bool, AppError> {
        ...
        for app in [
            ...
            let old_servers = match app {
                ...
                AppType::Antigravity => &self.mcp.antigravity.servers,
            };
        ...
        self.mcp.antigravity = McpConfig::default();
